...
begin
    ...
    /*  db_int                                         := nextval('dgmain.dg_id_seq');
      ...
      select * into db_int from key_lookup_temp_ld_fn(guest_rec);
		...
		then*/
		  ...
		  guest_rec.load_date                            := db_current_date;
			guest_rec.status                               := 'I';
		  guest_rec.name_prefix                          := (p_params -> 'Adults' -> i::text ->> 'name_prefix');
		  guest_rec.first_name                           := (p_params -> 'Adults' -> i::text ->> 'first_name');
		  guest_rec.middle_name                          := (p_params -> 'Adults' -> i::text ->> 'middle_name');
		  guest_rec.last_name                            := (p_params -> 'Adults' -> i::text ->> 'last_name');
		  guest_rec.name_suffix                          := (p_params -> 'Adults' -> i::text ->> 'name_suffix');
		  guest_rec.address1                             := coalesce((p_params -> 'Adults' -> i::text ->> 'address1')::text,'');
		  guest_rec.address2                             := coalesce((p_params -> 'Adults' -> i::text ->> 'address2')::text,'');
		  guest_rec.address3                             := coalesce((p_params -> 'Adults' -> i::text ->> 'address3')::text,'');
		  guest_rec.city                                 := coalesce((p_params -> 'Adults' -> i::text ->> 'city')::text,'');
		...
		then
		  ...
		  guest_rec.load_date                            := db_current_date;
			guest_rec.status                               := 'I';
		  guest_rec.name_prefix                          := (p_params -> 'Children' -> (i - adult_num)::text ->> 'name_prefix');
		  guest_rec.first_name                           := (p_params -> 'Children' -> (i - adult_num)::text ->> 'first_name');
		  guest_rec.middle_name                          := (p_params -> 'Children' -> (i - adult_num)::text ->> 'middle_name');
		  guest_rec.last_name                            := (p_params -> 'Children' -> (i - adult_num)::text ->> 'last_name');
		  guest_rec.name_suffix                          := (p_params -> 'Children' -> (i - adult_num)::text ->> 'name_suffix');
		  guest_rec.address1                             := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'address1')::text,'');
		  guest_rec.address2                             := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'address2')::text,'');
		  guest_rec.address3                             := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'address3')::text,'');
		  guest_rec.city                                 := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'city')::text,'');
		  guest_rec.state                                := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'state')::text,'');
		  guest_rec.zip                                  := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'zip')::text,'');
		  guest_rec.country                              := coalesce((p_params -> 'Children' -> (i - adult_num)::text ->> 'country')::text,'');
			guest_rec.dg_id                                := db_int;
  ...
  where rn = 1;
	...
	lead_rec.status                                := 'I';
	lead_rec.lead_id                               := coalesce(p_params ->> 'lead_id', v_lead_id);
